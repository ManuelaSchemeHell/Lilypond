...
german-scale-names =
#'(Dur Moll Ionisch Lokrisch Ã„olisch Mixolydisch Lydisch Phrygisch Dorisch)

french-scale-names =
#'(majeur mineur ionien locrien Ã©olien mixolydien lydien phrygien dorien)
...
#(define (note-name->french-string pitch)
   ...
   (let* ((name (ly:pitch-notename pitch))
     ...
     (string-append
      (vector-ref #("Do" "RÃ©" "Mi" "Fa" "Sol" "La" "Si") name)
      (cond
       ((= alteration FLAT) "-bÃ©mol")
       ((= alteration DOUBLE-FLAT) "-double bÃ©mol")
       ((= alteration SHARP) "-diÃ©se")
       ((= alteration DOUBLE-SHARP) "-double diÃ©se")
       (else "")))))
