...
rit =
\markup {
  \bold \italic \fontsize #-0.3 "ritardando"
}
...
%\ghostMusic \notes
%% the opposite of ghostMusic
%% extracts only music from argument
...
#(define-music-function (offset context grobname)
   (pair? symbol? symbol?)
   #{
     \once \override #(list context grobname 'outside-staff-priority) = #'()
     \once \override #(list context grobname 'Y-offset) = 0
     \once \override #(list context grobname 'extra-offset) = #offset
   #})

%% Aufruf: \allgShift #'(2 . 3) Staff NoteHead
   ...
   (let*
    ...
    (if (and (pair? script-stencil) (pair? (cdr script-stencil)))
        (let* ((rv (cdr script-stencil))
          ...
          (cond ((equal? script-type "dportato")
                 my-sil)
            ((or (equal? script-type "tenuto") (equal? script-type "uportato"))
             my-sil)
